<?
	...
	if($_POST['config'] == '2')
	{
		if($_POST['head_update'] == 'true')$_POST['head_update'] = 1;
		else $_POST['head_update'] = 0;
		
		if($_POST['head_update'] != $set_connection['head_update'])
		{
			if($_POST['head_update'] == 0)
			{
				$set_connection['head_update'] = 0;
				$out.=add_out('Вы отключили проверку обновлений.');
				$traffic -= 9;
			}
			else if($_POST['head_update'] == 1)
			{
				$set_connection['head_update'] = 1;
				$out.=add_out('Вы включили проверку обновлений.');
				$traffic += 9;
			}
			write_wb($_SERVER['DOCUMENT_ROOT'].'/database/users/'.$_SESSION['username'].'/set_connection.arr', serialize($set_connection));
		}
	}
	else if($_POST['config'] == '3')
	{
		$conf=json_decode(file_get_contents($_SERVER['DOCUMENT_ROOT'].'/database/users/'.$_SESSION['username'].'/chat_cfg.arr'));
		if($_POST['chat_t_or_b'] != $conf[0])
		{
			if($_POST['chat_t_or_b'] == '0')
			{
				$conf[0]='0';
				$out.=add_out('Новые сообщения снизу - готово!');
			}
			else if($_POST['chat_t_or_b'] == '1')
			{
				$conf[0]='1';
				$out.=add_out('Новые сообщения сверху - готово!');
			}
		}
		if($_POST['chat_smile_on_off'] != $conf[2])
		{
			switch ($_POST['chat_smile_on_off']){
			 case 'true':
				$conf[2]='true';
				$out.=add_out('Включить смайлы - готово!');
			        break;
			 case 'false':
				$conf[2]='false';
				$out.=add_out('Отключить смайлы - готово!');
			        break;
			}
		}
